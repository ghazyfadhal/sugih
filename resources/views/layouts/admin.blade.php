    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-white text-sugih-primary border-r border-gray-200 flex flex-col transition-all duration-300 relative z-20">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3 rounded-xl hover:bg-sugih-surface hover:text-sugih-gold transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-sugih-terracotta text-white font-bold' : 'text-sugih-secondary' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                    Dashboard
                </a>
                
                <a href="{{ route('admin.products.index') }}" class="flex items-center px-4 py-3 rounded-xl hover:bg-sugih-surface hover:text-sugih-gold transition-colors {{ request()->routeIs('admin.products.*') ? 'bg-sugih-terracotta text-white font-bold' : 'text-sugih-secondary' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                    Produk
                </a>
                
                <a href="{{ route('admin.articles.index') }}" class="flex items-center px-4 py-3 rounded-xl hover:bg-sugih-surface hover:text-sugih-gold transition-colors {{ request()->routeIs('admin.articles.*') ? 'bg-sugih-terracotta text-white font-bold' : 'text-sugih-secondary' }}">
                ...
                <a href="{{ route('admin.careers.index') }}" class="flex items-center px-4 py-3 rounded-xl hover:bg-sugih-surface hover:text-sugih-gold transition-colors {{ request()->routeIs('admin.careers.*') ? 'bg-sugih-terracotta text-white font-bold' : 'text-sugih-secondary' }}">
            ...
            dropZones.forEach(zone => {
                ...
                ['dragenter', 'dragover'].forEach(eventName => {
                    zone.addEventListener(eventName, () => {
                        zone.classList.add('border-sugih-green', 'bg-gray-100');
                    }, false);
                });

                ['dragleave', 'drop'].forEach(eventName => {
                    zone.addEventListener(eventName, () => {
                        zone.classList.remove('border-sugih-green', 'bg-gray-100');
                    }, false);
                });
